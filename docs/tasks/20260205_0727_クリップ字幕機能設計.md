...
- クリップの `transcript`（全文テキスト）
- クリップ範囲の `RefinedSentence[]`（タイムスタンプ付き文単位）
...
3. 元のタイムスタンプを基に、分割後のタイムスタンプを計算
4. 読みやすさを優先（1画面に表示する量を調整）

### プロンプト設計

```
あなたは動画字幕の編集者です。
以下の文字起こしテキストを、動画の字幕として表示するのに適した単位に分割してください。

## 入力データ
- transcript: クリップ全体のテキスト
- sentences: タイムスタンプ付きの文単位データ
...
### Phase 1: 共通インターフェース定義（先行作業）

**目的**: 並列実装のための契約を先に定める

| 作成物 | 内容 |
|--------|------|
| **Prisma スキーマ** | `ClipSubtitle` モデル、`Clip` への列追加 |
| **共有型定義** | `ClipSubtitleSegment`, `ClipSubtitle`, API Request/Response 型 |
| **Gateway インターフェース** | `ClipSubtitleRepositoryGateway`, `SubtitleComposerGateway` |
