...
```json
{
  ...
  "voices": {
    "default": {
      "modelId": "e58b0d7efca34eb38d5c4985e378abcb",
      "name": "デフォルト",
...
| フィールド | 型 | 説明 |
|------------|-----|------|
| modelId | string | Fish Audio音声モデルID |
| name | string | 表示名 |
...
1. `asset-registry.json`に`voices`セクション追加
2. `asset-registry.schema.json`にvoicesスキーマ追加
3. `AssetRegistryGateway`にvoice取得メソッド追加
4. Prismaスキーマに`voice_key`, `voice_speed`カラム追加
5. `ShortsScene`ドメインモデルに`voiceKey`, `voiceSpeed`追加
6. `TtsSynthesizeParams`に`speed`追加
7. `FishAudioTtsClient`でspeedをAPIリクエストに反映
8. シーン更新UseCaseで`voiceKey`, `voiceSpeed`の更新対応
9. 音声合成UseCaseでシーンの設定値を使用
...
| key | modelId | name | description |
|-----|---------|------|-------------|
| default | e58b0d7efca34eb38d5c4985e378abcb | デフォルト | 標準的な女性の声 |
